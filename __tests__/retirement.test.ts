...
describe('calculateRequiredSIP', () => {
  it('returns corpus/months when rate is 0', () => {
    expect(calculateRequiredSIP(1200000, 0, 0, 10)).toBeCloseTo(10000, 0);
  });

  it('forward simulation reaches target corpus', () => {
    const corpus = 5000000;
    const sip = calculateRequiredSIP(corpus, 0.12, 0.12, 20);
  ...
  it('higher returns = lower SIP', () => {
    expect(calculateRequiredSIP(5000000, 0.12, 0.12, 20)).toBeLessThan(calculateRequiredSIP(5000000, 0.10, 0.10, 20));
  });
});

describe('calculateTopUpSIP', () => {
  it('first-year SIP is lower than flat SIP', () => {
    const flat = calculateRequiredSIP(5000000, 0.12, 0.12, 20);
    const topUp = calculateTopUpSIP(5000000, 0.12, 0.12, 20, 0.10);
...
describe('calculateRetirement - integration', () => {
  ...
  it('handles retirement age equal to current age', () => {
    const r = calculateRetirement({ ...DEFAULT_INPUTS, currentAge: 60, retirementAge: 60, lifeExpectancy: 80 });
    expect(r.yearsToRetirement).toBe(0);
    expect(r.requiredMonthlySIP).toBe(0);
    expect(r.corpusDepletionTable.length).toBeGreaterThan(0);
  });

  it('handles retirement after life expectancy gracefully', () => {
    const r = calculateRetirement({ ...DEFAULT_INPUTS, currentAge: 70, retirementAge: 65, lifeExpectancy: 80 });
    expect(r.yearsToRetirement).toBeLessThanOrEqual(0);
    expect(r.requiredMonthlySIP).toBe(0);
  });

  it('monte carlo percentiles remain ordered for realistic inputs', () => {
    const r = calculateRetirement({ ...DEFAULT_INPUTS, postRetirementReturn: 0.08, lifeExpectancy: 90 });
    expect(r.monteCarlo.p10).toBeLessThanOrEqual(r.monteCarlo.p25);
    expect(r.monteCarlo.p25).toBeLessThanOrEqual(r.monteCarlo.p50);
    expect(r.monteCarlo.p50).toBeLessThanOrEqual(r.monteCarlo.p75);
    expect(r.monteCarlo.p75).toBeLessThanOrEqual(r.monteCarlo.p90);
  });

  it('creates a 3x3 sensitivity matrix centered on the base case', () => {
    const r = calculateRetirement({ ...DEFAULT_INPUTS, currentAge: 30, retirementAge: 60, lifeExpectancy: 85 });
    expect(r.sensitivityMatrix.length).toBe(3);
    expect(r.sensitivityMatrix.every(row => row.length === 3)).toBe(true);
    expect(r.sensitivityMatrix[1][1].isBase).toBe(true);
    expect(typeof r.sensitivityMatrix[0][0].corpus).toBe('number');
    expect(typeof r.sensitivityMatrix[2][2].sip).toBe('number');
    expect(r.sensitivityMatrix[0][0].inflationLabel).toMatch(/^[+-−]?[0-9]+%$|^Base$/);
    expect(r.sensitivityMatrix[0][0].returnLabel).toMatch(/^[+-−]?[0-9]+%$|^Base$/);
  });
