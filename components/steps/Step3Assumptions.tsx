...
export function Step3Assumptions({ inputs, updateInput, onNext, onBack }: Props) {
  ...
  const savingsAnalysis = useMemo(() => {
    ...
    try {
      ...
      const rawCorpus = calculateCorpus(totalRetirementExpense, inputs.postRetirementReturn, yearsInRet, blendedInfl, inputs.enableTax, inputs.taxRate);
      ...
      // Calculate real value (inflation-adjusted today's rupees)
      const realSavingsFV = savingsFV / Math.pow(1 + blendedInfl, yearsToRet);
      
      const fvDisplay = savingsFV >= 10000000 ? `₹${(savingsFV/10000000).toFixed(2)} Cr` : `₹${(savingsFV/100000).toFixed(1)} L`;
      const realFvDisplay = realSavingsFV >= 10000000 ? `₹${(realSavingsFV/10000000).toFixed(2)} Cr` : `₹${(realSavingsFV/100000).toFixed(1)} L`;
      
      return { savingsFV, coversPct, exceedsCorpus: savingsFV >= rawCorpus, fvDisplay, realFvDisplay };
    } catch { return null; }
  }, [inputs]);
  ...
  function resetDefaults() {
    ...
    updateInput('enableTax', false); updateInput('taxRate', 0.15); updateInput('enableGlidePath', false);
  }
    ...
    <section aria-labelledby="step3-heading">
      <div className="hb-card animate-fade-in">
        ...
        <div className="hb-card-body">
          ...
          {}
          <p className="hb-section-title">Retirement Lifestyle Preset</p>
          ...
          {}
          <p className="hb-section-title">Asset Allocation Glide Path</p>
          <div style={{ marginBottom: 20 }}>
            <label className="hb-toggle-label" htmlFor="enable-glidepath">
              <input id="enable-glidepath" type="checkbox" checked={inputs.enableGlidePath}
                onChange={e => updateInput('enableGlidePath', e.target.checked)}
                style={{ marginRight: 10, width: 16, height: 16, accentColor: 'var(--blue)', cursor: 'pointer' }} />
              <span style={{ fontWeight: 600, fontSize: 13, color: 'var(--text-secondary)' }}>Enable Linear Glide Path (Equity-to-Debt Shift)</span>
            </label>
            <p style={{ fontSize: 11, color: 'var(--text-muted)', paddingLeft: 26, marginTop: 4 }}>
              Gradually reduce investment returns from Pre-Retirement Return (e.g. {(inputs.preRetirementReturn * 100).toFixed(1)}%) linearly down to Post-Retirement Return (e.g. {(inputs.postRetirementReturn * 100).toFixed(1)}%) as you get closer to retirement. Models standard risk-reduction practice.
            </p>
          </div>

          {}
          <p className="hb-section-title">Taxation on Withdrawals</p>
          <div style={{ marginBottom: 16 }}>
            <label className="hb-toggle-label" htmlFor="enable-tax">
              <input id="enable-tax" type="checkbox" checked={inputs.enableTax}
                onChange={e => updateInput('enableTax', e.target.checked)}
                style={{ marginRight: 10, width: 16, height: 16, accentColor: 'var(--blue)', cursor: 'pointer' }} />
              <span style={{ fontWeight: 600, fontSize: 13, color: 'var(--text-secondary)' }}>Apply Flat Tax on Withdrawals</span>
            </label>
            <p style={{ fontSize: 11, color: 'var(--text-muted)', paddingLeft: 26, marginTop: 4 }}>
              Required corpus will scale up to cover tax liability on annual retirement withdrawals.
            </p>
          </div>
          {inputs.enableTax && sliderRow('tax-rate', 'Flat Tax Rate', 'Assumed flat rate (LTCG / Income slab blending).', inputs.taxRate * 100, 0, 30, 1, v => updateInput('taxRate', v / 100))}
          ...
          {savingsAnalysis && (
            <div style={{ fontSize: 11, color: 'var(--text-muted)', marginTop: -8, marginBottom: 16 }}>
              <p>Projected Future Value: <strong>{savingsAnalysis.fvDisplay}</strong> — covers {savingsAnalysis.coversPct.toFixed(0)}% of estimated corpus.</p>
              <p style={{ marginTop: 2, fontStyle: 'italic' }}>
                Inflation adjusted: Equivalent to <strong>{savingsAnalysis.realFvDisplay}</strong> in today&apos;s purchasing power (rupees).
              </p>
            </div>
          )}
