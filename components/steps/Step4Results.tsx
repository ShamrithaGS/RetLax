...
export function Step4Results({
  ...
  function handleCSVExport() {
    if (!results.corpusDepletionTable.length) return;
    const headers = ['Age', 'Opening Corpus (INR)', 'Returns Earned (INR)', 'Net Withdrawal (INR)', 'Tax Paid (INR)', 'Closing Corpus (INR)'];
    const rows = results.corpusDepletionTable.map(row => [
      row.age,
      Math.round(row.openingCorpus),
      Math.round(row.returns),
      Math.round(row.annualWithdrawal),
      Math.round(row.taxPaid),
      Math.round(Math.max(0, row.closingCorpus))
    ]);
    ...
    link.setAttribute('download', `retirement_depletion_table.csv`);
  ...
  function handleReportDownload() {
    const rows = results.corpusDepletionTable.slice(0, 5).map(row =>
      `Age ${row.age}: Opening ${formatCurrency(row.openingCorpus)}, Returns ${formatCurrency(row.returns)}, Withdrawal ${formatCurrency(row.annualWithdrawal)}, Tax ${formatCurrency(row.taxPaid)}, Closing ${formatCurrency(Math.max(0, row.closingCorpus))}`
    );

    const lines = [
      'RetLax Retirement Report',
      ...
      `- Current monthly expenses: ${formatCurrency(inputs.currentMonthlyExpenses * 12)}/year`,
      `- General inflation: ${(inputs.generalInflationRate * 100).toFixed(1)}%`,
      `- Medical inflation: ${(inputs.medicalInflationRate * 100).toFixed(1)}%`,
      `- Lifestyle inflation: ${(inputs.lifestyleInflationRate * 100).toFixed(1)}%`,
      `- Pre-retirement return: ${(inputs.preRetirementReturn * 100).toFixed(1)}%`,
      `- Post-retirement return: ${(inputs.postRetirementReturn * 100).toFixed(1)}%`,
      `- Existing savings: ${formatCurrency(inputs.existingSavings)}`,
      `- Withdrawal tax enabled: ${inputs.enableTax ? 'Yes' : 'No'}`,
      `- Glide path enabled: ${inputs.enableGlidePath ? 'Yes' : 'No'}`,
      '',
      'Summary results',
      `- Required retirement corpus: ${formatCurrency(results.retirementCorpus)}`,
      `- Required monthly SIP: ${formatCurrency(results.requiredMonthlySIP)}/mo`,
      `- Top-up SIP: ${formatCurrency(results.requiredMonthlySIPWithTopUp)}/mo`,
      `- Monte Carlo success rate: ${results.monteCarlo.successRate.toFixed(1)}%`,
      `- Monte Carlo p10/p90 ending corpus: ${formatCurrency(results.monteCarlo.p10)} / ${formatCurrency(results.monteCarlo.p90)}`,
      `- Break-even age: ${results.breakEvenAge ?? 'Not reached in horizon'}`,
      '',
      'Depletion table (first 5 years)',
      ...rows,
      '',
      'Disclaimer: This report is illustrative only. Past performance does not guarantee future results.',
    ];
    ...
    const link = document.createElement('a');
    link.setAttribute('href', url);
    link.setAttribute('download', `retirement_summary_report.txt`);
    link.style.visibility = 'hidden';
    document.body.appendChild(link);
    link.click();
    document.body.removeChild(link);
  ...
  const breakEvenMsg = results.breakEvenAge
    ? `Under base assumptions, your corpus runs out at age ${results.breakEvenAge}. If you live beyond this, you may face a financial shortfall.`
    ...
    <section aria-labelledby="step4-heading">
      <div className="animate-fade-in">
        ...
        <div id="r4-montecarlo" className="animate-fade-in animate-fade-in-delay-2">
          {}
          <div className="plain-language-card" style={{ marginBottom: 16, borderLeft: '4px solid var(--amber)' }}>
            <span className="plain-language-icon">⚠️</span>
            <div>
              <strong>Sequence-of-Returns Risk (SRR)</strong>
              <p style={{ fontSize: 12, marginTop: 4 }}>
                A bad run of returns in the first 5 years of retirement can deplete your corpus prematurely even if the average return over 25 years looks fine. This is because early losses lock in a smaller compounding base. The Monte Carlo simulation below models this sequence vulnerability.
              </p>
            </div>
          </div>
        ...
        {results.corpusDepletionTable.length > 0 && (
          <div className="expense-breakdown animate-fade-in animate-fade-in-delay-3" role="region" aria-label="Corpus depletion table">
            <div style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, marginBottom: 8, paddingBottom: 12, borderBottom: '1px solid var(--grey-border)', flexWrap: 'wrap' }}>
              <h3 style={{ margin: 0 }}>Year-by-Year Corpus Depletion — Inflation-Adjusted (Illustrative)</h3>
              <div style={{ display: 'flex', gap: 8 }}>
                <button type="button" onClick={handleCSVExport}
                  style={{ fontSize: 11, fontWeight: 600, color: 'var(--blue)', background: 'var(--blue-muted)', border: 'none', padding: '4px 10px', borderRadius: 'var(--radius-sm)', cursor: 'pointer' }}>
                  📥 Export CSV
                </button>
                <button type="button" onClick={handleReportDownload}
                  style={{ fontSize: 11, fontWeight: 600, color: 'var(--blue)', background: 'var(--blue-muted)', border: 'none', padding: '4px 10px', borderRadius: 'var(--radius-sm)', cursor: 'pointer' }}>
                  📄 Download report
                </button>
                ...
                <table className="depletion-table" aria-label="Corpus depletion table">
                  <thead>
                    <tr>
                      ...
                      {inputs.enableTax && <th scope="col">Tax Paid</th>}
                  ...
                  <tbody>
                    {results.corpusDepletionTable
                      ...
                      <tr key={row.age} className={row.closingCorpus < row.annualWithdrawal * 2 ? 'depletion-row-low' : undefined}>
                        ...
                        {inputs.enableTax && <td style={{ color: 'var(--red)' }}>{formatCurrency(row.taxPaid)}</td>}
        ...
        {}
        <div className="disclosure-box animate-fade-in animate-fade-in-delay-4" style={{ marginBottom: 16 }}>
          <button
            type="button"
          ...
          >
            <strong>Calculation Methodology &amp; Mathematical Formulas</strong>
            <span className="disclosure-toggle-hint" aria-hidden="true">
              {showMethodology ? '▲ Hide' : '▼ Show'}
            </span>
          </button>
          {showMethodology && (
            <div id="methodology-content" className="disclosure-content" style={{ fontSize: 12, lineHeight: 1.6 }}>
              <p>Below are the primary mathematical models used to evaluate your retirement corpus and SIP rates:</p>
              
              <div style={{ background: 'var(--blue-muted)', padding: 12, borderRadius: 6, margin: '12px 0', fontFamily: 'monospace' }}>
                <strong>1. Inflated Annual Expenses (Per Bucket)</strong>
                <p style={{ margin: '6px 0 0' }}>Basic: B_ret = B_now × (1 + i_g)^t</p>
                <p>Medical: M_ret = M_now × (1 + i_m)^t</p>
                <p>Lifestyle: L_ret = L_now × (1 + i_l)^t</p>
                <p style={{ fontSize: 10, color: 'var(--text-muted)', margin: 0 }}>where i_g, i_m, i_l are the general, medical, and lifestyle inflation rates, and t is years to retirement.</p>
              </div>

              <div style={{ background: 'var(--blue-muted)', padding: 12, borderRadius: 6, margin: '12px 0', fontFamily: 'monospace' }}>
                <strong>2. Retirement Corpus (Present Value of Growing Annuity)</strong>
                <p style={{ margin: '6px 0 0' }}>If returns equal inflation (r = g): C = W × t<sub>r</sub> / (1 + r)</p>
                <p>If returns vary (r ≠ g): C = W × [1 - ((1 + g) / (1 + r))<sup>t<sub>r</sub></sup>] / (r - g)</p>
                <p style={{ fontSize: 10, color: 'var(--text-muted)', margin: 0 }}>where W is annual expense, r is post-retirement return, g is blended inflation, and t<sub>r</sub> is years in retirement.</p>
                {inputs.enableTax && <p style={{ fontSize: 11, color: 'var(--red)', margin: '4px 0 0' }}>*With flat tax applied: W<sub>effective</sub> = W / (1 - taxRate)</p>}
              </div>

              <div style={{ background: 'var(--blue-muted)', padding: 12, borderRadius: 6, margin: '12px 0', fontFamily: 'monospace' }}>
                <strong>3. Linear Glide Path Return Shifting</strong>
                <p style={{ margin: '4px 0 0' }}>Compounding pre-retirement returns change month-by-month:</p>
                <p>r_annual(m) = r_pre - (m / totalMonths) × (r_pre - r_post)</p>
              </div>
            </div>
          )}
        </div>
          ...
          {showDisclosure && (
            <div id="disclosure-content" className="disclosure-content">
              <ul className="disclosure-list">
                {!inputs.enableTax && <li>Taxes on investment gains or withdrawals (Apply via assumptions toggle)</li>}
        ...
        <div id="r4-assumptions" className="assumptions-panel animate-fade-in animate-fade-in-delay-4 print-assumptions"
          ...
          <div className="assumptions-grid">
            {[
              ...
              ...(inputs.enableGlidePath ? [['Glide Path Shift', 'Enabled']] : []),
              ...(inputs.enableTax ? [['Flat Withdrawal Tax', `${(inputs.taxRate*100).toFixed(0)}%`]] : []),
        ...
        {!compareMode && (
          <div className="scenario-save-prompt animate-fade-in animate-fade-in-delay-4">
            ...
            <div>
              ...
              <p>Save this as Scenario A, then go back and change assumptions to compare side-by-side.</p>
            </div>
