...
export function MonteCarloChart({ mc, retirementAge, lifeExpectancy, postRetirementReturn }: Props) {
  ...
  return (
    ...
    >
      ...
      <div className="mc-footer-note" style={{ fontSize: 10, color: 'var(--text-muted)', lineHeight: 1.5, marginTop: 12 }}>
        <p><strong>Model Assumptions &amp; Disclosures:</strong></p>
        <p>• Assumes annual returns are Independent and Identically Distributed (i.i.d.) and does not account for multi-year economic cycles, market correlation, or mean-reversion.</p>
        <p>• Volatility is estimated dynamically based on the post-retirement return target (standard deviation scale: 7–12% volatility).</p>
      </div>
    </div>
  );
}
