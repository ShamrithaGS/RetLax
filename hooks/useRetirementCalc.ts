...
function encodeToURL(inputs: RetirementInputs): string {
  ...
  const p = new URLSearchParams({
    ...
    tx: inputs.enableTax ? '1' : '0', trt: String(inputs.taxRate),
    gp: inputs.enableGlidePath ? '1' : '0'
  });
  return p.toString();
}
...
function decodeFromURL(): Partial<RetirementInputs> {
  ...
  if (p.get('tx') !== null) partial.enableTax = p.get('tx') === '1';
  if (num('trt') !== undefined) partial.taxRate = num('trt')!;
  if (p.get('gp') !== null) partial.enableGlidePath = p.get('gp') === '1';
  return partial;
}
...
export function useRetirementCalc() {
  ...
  const updateInput = useCallback(<K extends keyof RetirementInputs>(
    ...
    if (value !== DEFAULT_INPUTS[key]) {
      setChangedKeys(prev => new Set(Array.from(prev).concat(key)));
    } else {
